...
public sealed record OfferProperty
{
    ...
    public OfferProperty(string name, string value)
    {
        ...
        if (value.Length < DataSchemaConstants.PropertyValueMinLength)
            throw new ArgumentException($"Property value must be at least {DataSchemaConstants.PropertyValueMinLength} characters.");

        if (value.Length > DataSchemaConstants.PropertyValueMaxLength)
            throw new ArgumentException($"Property value cannot exceed {DataSchemaConstants.PropertyValueMaxLength} characters.");
