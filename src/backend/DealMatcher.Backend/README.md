...
To update the database use this command from the Web project folder (replace `DealMatcher.Backend` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../DealMatcher.Backend.Infrastructure/DealMatcher.Backend.Infrastructure.csproj -s DealMatcher.Backend.Web.csproj
```
